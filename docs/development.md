Development of gww-network-trace plugin
===========================

This project uses [qgis_plugin_tools](https://github.com/timothy-holmes/qgis_plugin_tools) submodule,
so set git setting value: `git config --global submodule.recurse true`.

When cloning use `--recurse-submodules` like so:
`git clone --recurse-submodules https://github.com/timothy-holmes/gww-network-trace.git`
...
The code for the plugin is in the [gwwnetworktrace](../gww-network-trace) folder. Make sure you have required tools, such as
Qt with Qt Editor and Qt Linquist installed by following this
[tutorial](https://www.qgistutorials.com/en/docs/3/building_a_python_plugin.html#get-the-tools).

For building the plugin use platform independent [build.py](../gww-network-trace/build.py) script.

## Setting up development environment

To get started with the development, follow these steps:

1. Go to the  [gwwnetworktrace](../gww-network-trace) directory with a terminal
...
* they contain absolute imports:
    ```python
    from gwwnetworktrace.utils.exceptions import TestException # Good

    from ..utils.exceptions import TestException # Bad

    ```
* they will be found by [build.py](../gwwnetworktrace/build.py) script (`py_files` and `ui_files` values)

* you consider adding test files for the new functionality
## Deployment

Edit [build.py](../gwwnetworktrace/build.py) to contain working values for *profile*, *lrelease* and *pyrcc*. If you are
...
* Add this badge ![](https://github.com/timothy-holmes/gww-network-trace/workflows/Translations/badge.svg) to
...
The translation files are in [i18n](../gww-network-trace/resources/i18n) folder. Translatable content in python files is
